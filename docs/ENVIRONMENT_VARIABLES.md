...
- `ONNX_ASR_DECODER_DEVICE` (`cpu` | `model`, default `cpu`), `ONNX_ASR_PREPROCESSOR` (`cpu` | `model`, default `cpu`), `ONNX_ASR_CUDNN_ALGO_SEARCH` (`HEURISTIC` | `DEFAULT` | `EXHAUSTIVE`, default `HEURISTIC`), `ONNX_ASR_WARMUP` (default `true`): where the small pieces around the encoder run and how the CUDA provider is set up. A transducer (RNNT) runs its decoder and joiner once per 40 ms frame on tiny tensors, which is a fraction of a millisecond on the CPU and a kernel launch plus two copies on CUDA; the mel spectrogram is the same story; `EXHAUSTIVE` benchmarks the convolutions again for every new utterance length; the warm-up decodes 1, 3, 8 and 20 s of silence at start. The start-up line `Model ... initialized on ...` reports `decoder=`, `preprocessor=`, `cudnn_algo_search=` and `warmup=`.
- `SHERPA_VAD_MODEL_PATH`, `SHERPA_VAD_THRESHOLD`, `SHERPA_VAD_MIN_SILENCE_MS`, `SHERPA_VAD_MIN_SPEECH_MS`: the Silero VAD gate that cuts the caller's audio into phrases for the VAD-gated offline recognizers (Sherpa offline and `onnx_asr`).
- `SHERPA_OFFLINE_PREROLL_MS` (350), `SHERPA_OFFLINE_POSTROLL_MS` (300): stream audio taken before the VAD's start and after its end of a phrase before decoding. `SHERPA_OFFLINE_NORMALIZE_DBFS` (−20; `0` = off) and `SHERPA_OFFLINE_NORMALIZE_MAX_GAIN_DB` (24): the loudness a phrase is brought to and the largest boost allowed. `LOCAL_STT_RESAMPLER` (`fir` | `ratecv`): how 8 kHz client audio is brought to 16 kHz for these backends. All on the Env page under the backend's settings; see *Phrase segmenting* in [LOCAL_ONLY_SETUP.md](LOCAL_ONLY_SETUP.md#supported-stt-models).
