...
- **`extra_body` forwards vendor-specific fields to OpenAI-compatible LLMs:** the Chat Completions payload was built from a fixed set of keys and the provider model drops unknown ones, so an endpoint's own parameters had nowhere to go — `prompt_cache_key` for OpenAI/Mistral prompt caching (cached input billed at a fraction of the normal rate) or `chat_template_kwargs` for a vLLM chat template were silently dropped, and a stray `CACHE_KEY: prompt-1` in a provider block did nothing at all. A provider block, a pipeline's `options.llm`, or runtime options may now carry an `extra_body` mapping, shallow-merged in that order and forwarded verbatim. `model`, `messages`, `stream`, `tools` and `tool_choice` stay engine-owned and an `extra_body` entry for one of them is ignored with a warning. Each request that carries forwarded fields logs their names (never their values), so what reaches an endpoint is visible to the operator who configured it.
