...
- **`prompt_cache_key` for OpenAI-compatible LLM providers:** prompt caching on OpenAI and Mistral is opt-in and needs the key in the Chat Completions body, but the payload is built from a fixed set of fields, so a value configured on the provider never left the engine (unknown keys are dropped both by the provider model and by the payload builder — `CACHE_KEY: prompt-1` in a provider block did nothing). The provider block and a pipeline's `options.llm` now accept `prompt_cache_key`, forwarded as that body field when non-empty and omitted entirely otherwise, so deployments that do not set it send exactly what they sent before. Cached input tokens bill at a fraction of the normal rate; keep the value stable across calls that share a system prompt, bump it when the prompt changes, and keep secrets out of it.
