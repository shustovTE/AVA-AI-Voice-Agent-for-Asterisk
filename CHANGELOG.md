...
- **`extra_body` forwards vendor-specific fields to OpenAI-compatible LLMs:** the Chat Completions payload was built from a fixed set of keys, so an endpoint's own parameters had nowhere to go — `prompt_cache_key` for OpenAI/Mistral prompt caching (cached input billed at a fraction of the normal rate) or `chat_template_kwargs` for a vLLM chat template were silently dropped from the config. A provider block, a pipeline's `options.llm`, or runtime options may now carry an `extra_body` mapping, shallow-merged in that order and forwarded verbatim; `model`, `messages`, `stream`, `tools` and `tool_choice` stay engine-owned and an `extra_body` entry for one of them is ignored with a warning. Note that the shipped `local_hybrid` pipeline has always carried an (until now inert) `extra_body.chat_template_kwargs` block, which this makes live — repoint that pipeline at an endpoint that rejects unknown fields and the request will fail where it previously passed.
